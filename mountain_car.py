...
for i_episode in range(10000):
    ...
    if(len(replay_memory) > 1000):
      ...
      X_replay = np.zeros([500,5])
      Y_replay = np.zeros([500])
      for i in range(1,500):
          ...
          if(np.abs(replay.s_next[0])>2.4 or np.abs(replay.s_next[2])>0.20 ): #If it was a terminal state
            Y_replay[i] = replay.r
          else:
            Y_replay[i] = replay.r+gamma*model.predict(np.array([np.append(replay.s_next,followup_best_action)]))
          print(replay.r,model.predict(np.array([np.append(replay.s_next,followup_best_action)])))
